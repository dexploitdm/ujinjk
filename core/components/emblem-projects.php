...
add_action( 'init', 'true_register_post_type_init_projects_emblem' ); // Использовать функцию только внутри хука init

function true_register_post_type_init_projects_emblem() {
    $labels = array(
        'name' => 'Емблемы проектов',
        ...
        'menu_name' => 'Емблемы проектов' // ссылка в меню в админке
    );
    $args = array(
        'labels' => $labels,
        'public' => true,
    ...
    register_post_type('projects_emblem', $args);
}
